...
**Target Alive Check на этапе Initiation — предотвращение потери кулдауна на мёртвых целях.**
- `CombatSystem::initiateSkillUsage` — добавлена ранняя проверка живости цели (моб: `mobData.isDead`, игрок: `charData.isDead`) до установки кулдауна. Если цель мертва, возвращается `errorMessage = "Target is dead"`, кулдаун и мана не тратятся. Ранее проверка была только в `executeSkillUsage`, то есть кулдаун уже успевал установиться до выяснения, что цель мертва.

**PvP Guard на этапе Initiation — блокировка PvP-скилов до расхода ресурсов.**
- `initiateSkillUsage` — добавлена ранняя блокировка damage/debuff скилов по другим игрокам (`"PvP is not available"`) до установки кулдауна. Ранее PvP-гард был только в `executeSkillUsage`, что приводило к потере кулдауна и маны при попытке PvP-атаки.
...
Fixes:
 ...
 - Исправления багов с очисткой ghost персонажей (краш, пропал интернет, т.п.) и поведением при подключении.
 - Исправили хардкод в чанк сервере при передаче айпи.
...
Docker:
- `docker-compose.yml`: хардкод-переменные заменены на `env_file: .env`, `GAME_SERVER_HOST=game-server` оверрайдом.
...
================
New:
- Добавлена поддержка прыжка для игроков.
