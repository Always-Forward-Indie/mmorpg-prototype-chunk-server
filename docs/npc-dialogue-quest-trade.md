...
| `level`          | int    | Уровень |
| `npcType`        | string | `general`, `quest_giver`, `vendor` |
    ...
    "npcsSpawn": [
      {
        ...
        "position": { "x": 1200.0, "y": 750.0, "z": 0.0, "rotationZ": 1.57 },
        "attributes": [
      ...
      },
      {
        ...
        "position": { "x": 1350.0, "y": 800.0, "z": 0.0, "rotationZ": 3.14 },
        "attributes": []
      }
    ]
  }
...
{
  "header": { "eventType": "QUEST_UPDATE" },
  "body": { "questId": 5, "state": "completed", "currentStep": 1 }
}
```
...
{
  "header": { "eventType": "QUEST_UPDATE" },
  "body": { "questId": 5, "state": "turned_in" }
}
```
...
| `clientQuestKey` | string | Ключ квеста для локализации (`quest.slug`) |
...
| `QUEST_UPDATE`      | Любое изменение квеста | `questId`, `state`, `progress`, `required` |
