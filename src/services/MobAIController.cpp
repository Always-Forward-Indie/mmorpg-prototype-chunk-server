...
MobAIController::checkAndTriggerFlee(MobDataStruct &mob, MobMovementData &movementData, float currentTime, int attackerPlayerId)
{
    ...
    {
        auto freshData = mobInstances_.getMobInstance(mob.uid);
        if (freshData.uid > 0)
    ...
    // --- Compute flee destination (pure): away from the last attacker ------
    bool hasAttacker = false;
    PositionStruct attackerPos;
    if (attackerPlayerId > 0)
    {
        auto attacker = characters_.getCharacterById(attackerPlayerId);
        if (attacker.characterId > 0)
        {
            hasAttacker = true;
            attackerPos = attacker.characterPosition;
        }
    }
    movementData.fleeTargetPosition =
        MobAIFormulas::computeFleeTarget(mob.position, hasAttacker, attackerPos, mob.aggroRange, mob.chaseMultiplier);
    ...
    mobMovement_.updateMobMovementData(mob.uid, movementData);
    mobMovement_.forceMobStateUpdate(mob.uid);
...
MobAIController::MobAIController(CharacterManager &characters,
...
{
    // Need template skills — empty template means base attack.
    // getMobById returns by value; bind to a local copy so we own the data..
    MobDataStruct tmpl = mobs_.getMobById(mob.id);
    if (tmpl.skills.empty())
        return std::nullopt;

    float currentTime = getCurrentGameTime();

    // Pure scoring (range filter, per-skill cooldown, ability-first).
    const int chosenIdx = MobAIFormulas::selectMobSkillIndex(
        tmpl.skills, movementData.skillLastUsedTime, currentTime, distanceToTarget);
    if (chosenIdx < 0)
        return std::nullopt;
    return tmpl.skills[static_cast<size_t>(chosenIdx)]; // value copy, safe after tmpl is destroyed
}

bool
MobAIController::isTargetAlive(int targetPlayerId)
{
    auto player = characters_.getCharacterById(targetPlayerId);
...
{
    auto target = characters_.getCharacterById(targetPlayerId);
    if (target.characterId == 0)
        return 0;

    auto nearbyMobs = mobInstances_.getMobsInRange(
...
MobAIController::handleMobAttacked(int mobUID, int attackerPlayerId, int damage)
{
    ...
    auto movementData = mobMovement_.getMobMovementData(mobUID);
        ...
        movementData.combatState == MobCombatState::FLEEING)
    {
        if (damage > 0)
        {
            ...
            mobMovement_.updateMobMovementData(mobUID, movementData);
        }
        logger_.log("[INFO] Mob UID: " + std::to_string(mobUID) +
    ...
    if (damage > 0)
    {
        ...
        if (mob.uid > 0 && mob.isSocial)
        {
            ...
            auto neighbors = mobInstances_.getMobsInRange(
            ...
            for (const auto &neighbor : neighbors)
            {
                ...
                auto neighborMov = mobMovement_.getMobMovementData(neighbor.uid);
...
MobAIController::handlePlayerAggro(MobDataStruct &mob, const SpawnZoneStruct &zone, MobMovementData &movementData)
{
    ...
    if (movementData.targetPlayerId > 0)
    {
        if (!isTargetAlive(movementData.targetPlayerId))
        {
            ...
            mobMovement_.updateMobMovementData(mob.uid, movementData);
            mobMovement_.sendMobTargetLost(mob, lostTargetId);
    ...
    {
        auto nearbyPlayers = characters_.getCharactersInZone(
            mob.position.positionX,
            mob.position.positionY,
            mob.aggroRange);

        if (!nearbyPlayers.empty() && mobMovement_.canSearchNewTargets(mob.position, zone))
        {
    ...
    if (mob.archetypeType == MobArchetype::CASTER && movementData.targetPlayerId > 0)
    {
...
MobAIController::updateMobCombatState(MobDataStruct &mob, MobMovementData &movementData, float currentTime)
{
            ...
            mob.currentHealth < mob.maxHealth && mob.maxHealth > 0)
        {
            ...
            else
            {
                ...
                if (regenDelta >= 1.0f)
                {
                    ...
                    if (healAmount > 0)
                    {
                        auto result = mobInstances_.applyHealToMob(mob.uid, healAmount);
                        if (result.success)
                        {
                            movementData.lastRegenTime = currentTime;
                            mobMovement_.updateMobMovementData(mob.uid, movementData);
    ...
    case MobCombatState::CHASING:
        ...
        {
            auto targetPlayer = characters_.getCharacterById(movementData.targetPlayerId);
            if (targetPlayer.characterId == 0)
            {
                ...
                logger_.log("[COMBAT] Mob " + std::to_string(mob.uid) +
                            " target no longer exists, returning to spawn");
            }
            else if (!isTargetAlive(movementData.targetPlayerId))
            {
                ...
                mobMovement_.sendMobTargetLost(mob, lostTargetId);
                logger_.log("[COMBAT] Mob " + std::to_string(mob.uid) +
                            " target is dead, returning to spawn");
            }
            else
            {
                ...
                if (decayDelta >= 0.1f) // update at most every 100 ms
                {
                    ...
                    const float decayFactor = MobAIFormulas::threatDecayFactor(decayDelta);
                ...
                {
                    const int maxMeleeSlots =
                        MobAIFormulas::maxMeleeSlots(mob.attackRange, mob.radius);
                ...
                // fight after maxChaseTime and can never kill a passive player.
                if (distance > mob.attackRange && timeSinceChasing > maxChaseTime)
                {
                    int lostTargetId = movementData.targetPlayerId;
                    movementData.targetPlayerId = 0;
                    movementData.combatState = MobCombatState::RETURNING;
                ...
                else
                {
                    if (distance <= mob.attackRange)
                    {
                        ...
                        if (chosenOpt)
                        {
                            ...
                            // Send combatInitiation and combatResult immediately (no cast delay).
                            if (combatSystem_)
                        ...
                        else
                        {
                            ...
                            // Broadcast a fallback combatInitiation and apply damage immediately.
    ...
    case MobCombatState::PREPARING_ATTACK:
        ...
        if (timeSinceStateChange >= movementData.attackPrepareTime)
        {
            ...
            {
                // Target died during cast — abort and return to spawn.
                int lostTargetId = movementData.targetPlayerId;
                movementData.targetPlayerId = 0;
                movementData.combatState = MobCombatState::RETURNING;
                movementData.isReturningToSpawn = true;
                ...
                mobMovement_.sendMobTargetLost(mob, lostTargetId);
                logger_.log("[COMBAT] Mob " + std::to_string(mob.uid) +
                            " target is dead during prepare attack, returning to spawn");
            }
            else if (movementData.targetPlayerId > 0)
            {
                // Damage was already applied at CHASING→PREPARING_ATTACK entry.
                // Just advance to ATTACKING (swing animation phase).
    ...
    case MobCombatState::ATTACKING:
        // combatResult was sent at CHASING→PREPARING_ATTACK (together with combatInitiation).
        ...
        if (timeSinceStateChange >= movementData.postAttackCooldown)
        {
            ...
            if (movementData.targetPlayerId > 0)
            {
                if (!isTargetAlive(movementData.targetPlayerId))
                {
                    ...
                    mobMovement_.sendMobTargetLost(mob, lostTargetId);
                    logger_.log("[COMBAT] Mob " + std::to_string(mob.uid) +
                                " target is dead after cooldown, returning to spawn");
                }
                else
                {
                    movementData.combatState = MobCombatState::CHASING;
