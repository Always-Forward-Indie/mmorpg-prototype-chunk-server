...
std::vector<CharacterAttributeStruct> JSONParser::parseCharacterAttributesList(const char* data, size_t length)
{
    nlohmann::json jsonData = nlohmann::json::parse(data, data + length);
...
std::vector<CharacterDataStruct> JSONParser::parseCharactersList(const char* data, size_t length)
{
    nlohmann::json jsonData = nlohmann::json::parse(data, data + length);
...
// parse chunk info
ChunkInfoStruct JSONParser::parseChunkInfo(const char* data, size_t length)
{
    nlohmann::json jsonData = nlohmann::json::parse(data, data + length);
...
std::vector<SpawnZoneStruct> JSONParser::parseSpawnZonesList(const char* data, size_t length)
{
    nlohmann::json jsonData = nlohmann::json::parse(data, data + length);
    std::vector<SpawnZoneStruct> spawnZonesList;

    if (jsonData.contains("body") && jsonData["body"].is_object() &&
        jsonData["body"].contains("spawnZonesList") && jsonData["body"]["spawnZonesList"].is_array()) {
        for (const auto& zone : jsonData["body"]["spawnZonesList"]) {
            ...
            if (zone.contains("spawnCount") && zone["spawnCount"].is_number_integer()) {
                zoneData.spawnCount = zone["spawnCount"].get<int>();
            }
...
// parse mobs list
std::vector<MobDataStruct> JSONParser::parseMobsList(const char* data, size_t length)
{
    nlohmann::json jsonData = nlohmann::json::parse(data, data + length);
...
std::vector<MobAttributeStruct> JSONParser::parseMobsAttributesList(const char* data, size_t length)
{
    nlohmann::json jsonData = nlohmann::json::parse(data, data + length);
